...
export class TOC {
  ...
  private renderToMd(tree: Entry, linkDir = false): string {
    if (!tree.children) {
      return `- [${this.niceName(path.basename(tree.name, '.md'))}](${tree.link.replace(
        / /g,
        '%20',
      )})`;
    } else {
  ...
  index(path: string, leftPad: number, baseLink = '') {
    let padding = new Array(leftPad + 1).join(' ');
    let root = this.buildTree(path, '', baseLink);
    return this.renderToMd(root)
      .split('\n')
      .map(i => i.substr(2))
      .join('\n' + padding);
  }
